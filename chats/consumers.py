...
class MyAsyncWebsocketConsumer(AsyncWebsocketConsumer):
    ...
    async def connect(self):
        ...
        if self.user.username>self.other_user.username:
            self.room_name = f"{self.user.username}_{self.other_user.username}"
        else:
            self.room_name = f"{self.other_user.username}_{self.user.username}"
    ...
    async def receive(self, text_data=None, bytes_data=None):
        ...
        # await database_sync_to_async(Message.objects.create)(sender=self.other_user, receiver=self.user, content=message)
